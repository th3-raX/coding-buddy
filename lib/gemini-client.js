 ...
 * 
 * Uses gemini-3.1-flash with JSON mode for structured, reliable output.
...
async function reviewCode(code, language) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403 || response.status === 401) {
      throw new Error('AI service authentication error. Please contact the administrator.');
    }
  ...
  if (!text) {
    ...
    if (blockReason === 'SAFETY') {
      throw new Error('The AI flagged this content. Please modify your code and try again.');
    }
